...
def alpso(dimensions,constraints,neqcons,xtype,x0,xmin,xmax,swarmsize,nhn,
    ...
    while ((k_out < maxOutIter) and (stop_main_flag == 0)):
        ...
        if (fileout == 1) or (fileout == 3):
            ...
            ofile.write("\tF = %.16g\n" %(float(swarm_f)))
            ofile.write("\nAUGMENTED LAGRANGIAN TERM VALUE:\n")
            ofile.write("\tL = %.16g\n" %(float(swarm_L)))
            if (constraints > 0):
                # Equality Constraints
        ...
        if (stopCriteria == 1):
            ...
            # Modified by Jun Zhu
            # Show why the optimization stops
            #
            ...
            if abs(global_distance[0]-global_distance[stopIters-1]) <= \
                ...
                print("\nOptimization converged because of global distance and "
                      "relative tolerance!")
                stop_criteria_flag = 1

            elif abs(global_L[0]-global_L[stopIters-1]) <= atol:
                print "\nOptimization converged because of absolute tolerance!"
                stop_criteria_flag = 1
    ...
    # Added by Jun Zhu
    if k_out == maxOutIter:
        print("\nMaximum outer iteration reached!")
